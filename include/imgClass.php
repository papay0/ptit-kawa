...
class Img{
  ...
  static function compress_image($source_url, $destination_url, $quality) {
  $info = getimagesize($source_url);
    // echo "information image upload = \n".print_r($info)."\n";
  ...
  elseif ($info['mime'] == 'image/png') 
  {
    ...
    //echo "\n\n image from png/gif etc... = ".$image."\n";
 
  //save file
    
    $ext =  strtolower(substr($destination_url, -3));
                    if ($ext == "png")
                    {
                        $destination_url = (substr($destination_url, 0, -3)."jpg");
                       // $fileTransform = (substr($fileTransform, 0, -3)."jpg");
                    }
    
    // echo "\n je deplace ".$image." ici ==> ".$destination_url." \n";
